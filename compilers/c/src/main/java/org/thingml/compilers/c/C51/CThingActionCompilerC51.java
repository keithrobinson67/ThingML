 ...
 */
public class CThingActionCompilerC51 extends CThingActionCompiler {

	@Override
    public void generate(CastExpression exp, StringBuilder builder, Context ctx) {
		if (AnnotatedElementHelper.isDefined(exp.getType(), "8051_type", "String")) {
    ...
    public void generate(PrintAction action, StringBuilder builder, Context ctx) {
        for(Expression e : action.getMsg()) {
            final StringBuilder b = new StringBuilder();
            generate(e, b, ctx);
        	if (AnnotatedElementHelper.hasAnnotation(ctx.getCurrentConfiguration(), "8051_stdout")) {
                builder.append(AnnotatedElementHelper.annotation(ctx.getCurrentConfiguration(), "8051_stdout").iterator().next() + ".print(" + b.toString() + ");\n");
            } else {
        ...
        if (action.isLine()) {
        	if (AnnotatedElementHelper.hasAnnotation(ctx.getCurrentConfiguration(), "8051_stdout")) {
                builder.append(AnnotatedElementHelper.annotation(ctx.getCurrentConfiguration(), "8051_stdout").iterator().next() + ".print(\"\\n\");\n");
            } else {
